@php
    /** @var \Melasistema\HydeEventsModule\Pages\MarkdownEvent $page  */
...
<figure aria-label="Cover image" itemprop="image" itemscope itemtype="https://schema.org/ImageObject" role="doc-cover">
    <img src="{{ $image->getSource() }}" alt="{{ $image->getAltText() ?? '' }}" title="{{ $image->getTitleText() ?? '' }}" itemprop="image" class="mb-0 rounded">
    <figcaption aria-label="Image caption" itemprop="caption">
        @if($image->hasAuthorName())
            <span>Image by</span>
            <span itemprop="creator" itemscope="" itemtype="https://schema.org/Person">
                @if($image->hasAuthorUrl())
                    <a href="{{ $image->getAuthorUrl() }}" rel="author noopener nofollow" itemprop="url" target="_blank">
